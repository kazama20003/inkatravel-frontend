...
import { CalendarIcon, Users, X, Edit } from "lucide-react"
import { cn } from "@/lib/utils"
import { toast } from "sonner"
import { format } from "date-fns"
import { es } from "date-fns/locale"
import type { Order, UpdateOrderDto, UserOption, TourSelectionOption } from "@/types/order"
import { updateOrder, updateOrderStatus, getUsers, getToursForSelection } from "@/lib/orders-api"
...
export function OrderEditDialog({ order, onOrderUpdated, trigger }: OrderEditDialogProps) {
  const [open, setOpen] = useState(false)
  const [loading, setLoading] = useState(false)
  const [tours, setTours] = useState<TourSelectionOption[]>([])
  const [users, setUsers] = useState<UserOption[]>([])
  const [selectedDate, setSelectedDate] = useState<Date>()
  const [currentStatus, setCurrentStatus] = useState<"created" | "confirmed" | "cancelled" | "completed">("created")
  ...
  useEffect(() => {
    if (order && open) {
      ...
      setSelectedDate(new Date(order.startDate))
      fetchTours()
      fetchUsers()
    }
  }, [order, open])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Primero actualizar los datos de la orden (sin status)
      const orderData: UpdateOrderDto = {
        ...formData,
        startDate: selectedDate.toISOString(),
      }

      console.log("Actualizando datos de la orden:", orderData) // Para debug
      await updateOrder(order._id, orderData)

      // Si el status cambió, actualizarlo por separado
      if (currentStatus !== order.status) {
        console.log("Actualizando status:", currentStatus) // Para debug
        await updateOrderStatus(order._id, currentStatus)
      }
      ...
      console.error("Error al actualizar reserva:", error) // Para debug
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="w-[95vw] max-w-2xl max-h-[95vh] sm:max-h-[90vh]">
        <DialogHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <DialogTitle className="text-lg sm:text-xl">Editar Reserva</DialogTitle>
        ...
        <ScrollArea className="max-h-[calc(95vh-120px)] sm:max-h-[calc(90vh-120px)] pr-4">
          <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
            ...
            <div className="space-y-2">
              ...
              <Select
                value={currentStatus}
                onValueChange={(value: "created" | "confirmed" | "cancelled" | "completed") => setCurrentStatus(value)}
              >
                <SelectTrigger>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                  <div>
                    ...
                    <p className="text-xs sm:text-sm text-muted-foreground">
                      Estado: <span className="capitalize">{currentStatus}</span>
                    </p>
                  </div>
