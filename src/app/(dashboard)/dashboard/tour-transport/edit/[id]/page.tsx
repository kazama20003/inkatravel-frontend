...
import { ArrowLeft, Plus, Trash2, Save, Clock, Loader2 } from "lucide-react"
import type { UpdateTourTransportDto } from "@/types/tour-transport"
...
export default function EditTourTransportPage() {
  ...
  const { register, control, handleSubmit, watch, setValue, reset } = useForm<UpdateTourTransportDto>({
    defaultValues: {
      title: {},
      description: {},
      termsAndConditions: {},
  ...
  useEffect(() => {
    const fetchTourTransport = async () => {
      try {
        setLoading(true)
        const response = await api.get(`/tour-transport/all/${id}`)
        const data = response.data

        // Convert API time format (07:30 AM) to HTML time input format (07:30)
        const convertAPITimeToInputTime = (timeStr?: string): string => {
          if (!timeStr) return ""
        ...
        reset({
          ...
          departureTime: convertAPITimeToInputTime(data.departureTime),
          arrivalTime: convertAPITimeToInputTime(data.arrivalTime),
        ...
        toast.success("Datos cargados", {
          description: "El tour de transporte se ha cargado correctamente.",
        })
      } catch (err) {
        console.error("Error fetching transport:", err)
        toast.error("Error al cargar", {
          description: "No se pudo cargar el tour de transporte.",
        })
  ...
  const onSubmit = async (data: UpdateTourTransportDto) => {
    try {
      setSubmitting(true)
      const formattedData = {
        ...data,
      ...
      await api.patch(`/tour-transport/${id}`, formattedData)
      toast.success("Actualizado exitosamente", {
        description: "El tour de transporte ha sido actualizado.",
      })
      router.push("/dashboard/tour-transport")
    } catch (err) {
      console.error("Error updating transport:", err)
      toast.error("Error al actualizar", {
        description: "No se pudo actualizar el tour de transporte. Intenta nuevamente.",
      })
    } finally {
      setSubmitting(false)
    }
      ...
      <SidebarInset>
          ...
          <div className="flex items-center gap-2 px-4">
            ...
            <Breadcrumb>
              <BreadcrumbList>
                ...
                <BreadcrumbSeparator className="hidden md:block" />
                <BreadcrumbItem>
                  <BreadcrumbPage>Editar Transporte</BreadcrumbPage>
                </BreadcrumbItem>
              </BreadcrumbList>
            </Breadcrumb>
          </div>
        ...
        <div className="flex flex-1 items-center justify-center">
          <div className="flex flex-col items-center gap-4">
            ...
            <p className="text-muted-foreground">Cargando datos del tour...</p>
          </div>
        </div>
      </SidebarInset>
    )
        ...
        <div className="mx-auto w-full max-w-7xl px-4 py-8 md:px-6 lg:px-8">
          <div className="flex items-center justify-between mb-8">
            <div className="space-y-1">
              <h1 className="text-3xl font-bold tracking-tight">Editar Tour de Transporte</h1>
              <p className="text-muted-foreground">Actualiza la información del tour de transporte</p>
            </div>
